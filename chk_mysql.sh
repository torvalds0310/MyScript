...
$mysqladmin -u$user -p$passwd -h$host status 1>>/dev/null 2>/home/mysqlstatus.log

if [ $? -ne 0 ]; then
	echo "$currdate" > /home/mysqlstatus.log
	echo "Mysql is not runing, please run command "/etc/init.d/mysqld start" to start it." > /home/mysqlstatus.log
...
$mysql -u$user -p$passwd -h$host -e "insert into test.mytest values(1,'test');" 1>/dev/null 2>/home/mysqlinsert.log

if [ $? -ne 0 ]; then
	echo "$currdate" >> /home/mysqlinsert.log
	echo "MySQL is not functional as can not insert data into table test.mytest."
